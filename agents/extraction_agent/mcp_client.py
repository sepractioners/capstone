"""Thin MCP client: spawns the CLM MCP server over stdio and calls its
tools. This is the only place the extraction agent (or a future UI built
on this package) talks to the domain - everything upstream just produces
a ContractCandidate, and everything downstream reads back through here.
"""
...
def _server_params(database_path: Optional[str]) -> StdioServerParameters:
    """Build stdio launch parameters for the local CLM MCP server."""
    env = dict(os.environ)
    if database_path:
        env["CLM_DATABASE_PATH"] = database_path
    return StdioServerParameters(command=sys.executable, args=["-m", "clm_mcp_server.server"], env=env)
